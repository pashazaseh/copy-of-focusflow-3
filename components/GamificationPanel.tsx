                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="date" value={expiryDate} onChange={e => setExpiryDate(e.target.value)} className={`w-full p-2 rounded-lg border focus:outline-none focus:ring-2 ${isCyberpunk ? 'bg-black border-[#00f0ff]/30 text-[#00f0ff] focus:ring-[#00f0ff] [color-scheme:dark]' : 'bg-slate-800 border-slate-600 text-white focus:ring-blue-500 [color-scheme:light] dark:[color-scheme:dark]'}`} />
                        </div>
                    </div>
                    <div>
...
export const GamificationPanel: React.FC<GamificationPanelProps> = ({ activeProject, userState, isCyberpunk, projects, onSelectProject }) => {
  ...
  if (activeProject && !activeProject.streak) {
    console.error("Project is missing streak object:", activeProject);
  }
  ...
  const finalizeSpin = () => {
      ...
      // Deduct spin from project if applicable
      if (activeProject && activeProject.spinsAvailable && activeProject.spinsAvailable > 0) {
          const updated = { ...activeProject, spinsAvailable: activeProject.spinsAvailable - 1 };
  ...
  const projectAchievements = useMemo(() => {
      ...
      return PROJECT_TROPHIES.map(t => ({
          id: t.id,
          title: t.name,
          description: t.description,
          icon: t.icon,
          isUnlocked: unlockedSet.has(t.id),
          rewardConfig: { gems: 0, rarity: 'common' as const },
          progress: unlockedSet.has(t.id) ? 100 : 0
      }));
  }, [activeProject]);
      ...
      <div className="p-6 h-full overflow-y-auto custom-scrollbar relative z-10">
        <div className="max-w-7xl mx-auto space-y-8 animate-fade-in-up">
                ...
                <div className="flex gap-4">
                    <div className={`flex flex-col items-center p-3 rounded-2xl border min-w-[100px] ${isCyberpunk ? 'bg-black border-[#ff00ff]/30' : 'bg-black/40 border-red-500/30'}`}>
                        ...
                        <div className={`text-2xl font-black drop-shadow-[0_0_8px_rgba(239,68,68,0.5)] ${isCyberpunk ? 'text-[#ff00ff] drop-shadow-[0_0_8px_rgba(255,0,255,0.5)]' : 'text-red-500'}`}>
                            {activeProject ? (activeProject.streak?.current || 0) : streak} <span className="text-sm">🔥</span>
                        </div>
                    </div>
            ...
            <div className={`flex p-1 rounded-2xl mb-8 transition-all duration-300 ${isCyberpunk ? 'bg-black/40 border border-[#00f0ff]/20 shadow-[0_0_15px_rgba(0,240,255,0.1)]' : 'bg-gray-100 dark:bg-white/5'}`}>
                {(['shop', 'challenges', 'earn'] as const).map((tab) => (
                    <button
                        key={tab}
                                ...
                                <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <button 
                                            ...
                                            disabled={!activeProject.spinsAvailable || activeProject.spinsAvailable <= 0}
                                            className={`px-8 py-3 font-black text-lg rounded-xl shadow-lg transform hover:scale-105 transition-all disabled:opacity-50 disabled:cursor-not-allowed ${isCyberpunk ? 'bg-[#00f0ff] text-black shadow-[0_0_15px_rgba(0,240,255,0.5)]' : 'bg-gradient-to-r from-yellow-400 to-orange-500 hover:from-yellow-300 hover:to-orange-400 text-black shadow-orange-500/20'}`}
                                        >
                                            SPIN ({activeProject.spinsAvailable || 0})
                                        </button>
                                    </div>
                                </div>
                    ...
                    <div className={`rounded-3xl p-8 border ${isCyberpunk ? 'bg-[#0a0a0a] border-[#00f0ff]/20' : 'bg-white dark:bg-slate-800/50 border-gray-200 dark:border-white/5'}`}>
                        <div className="flex justify-between items-center mb-6">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button 
                                    onClick={() => { 
                                        if(isAddChallengeMenuOpen) {
                                            handleCancelEdit();
                                        } else {
                                            setIsAddChallengeMenuOpen(true);
                                        }
                                    }} 
                                    className={`p-2 rounded-xl transition-all ${isAddChallengeMenuOpen ? (isCyberpunk ? 'bg-[#00f0ff] text-black' : 'bg-blue-600 text-white') : (isCyberpunk ? 'bg-[#00f0ff]/10 text-[#00f0ff] hover:bg-[#00f0ff]/20' : 'bg-gray-100 dark:bg-slate-700 text-gray-600 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-slate-600')}`}
                                >
